...
class LoginAction extends Action
{
        ...
        public function login()
        {
                ...
                $userArr = $_SESSION['userinfo'] ? $_SESSION['userinfo'] : $_SESSION['cuserinfo'];
                if (!empty($userArr))
                {
        ...
        public function judge_login()
        {
                if ($_POST['type'] == 1)
                {
                        ...
                        if ($userArr['userid'] > 0)
                        {
                                ...
                                if ($userArr['flag'] == "1")
                                {
                                        ...
                                        //存进session信息                                
                                        $_SESSION['cuserinfo'] = array("userid" => $check_user['id'], "username" => $data['username']);
                                        //setcookie("cuserinfo", serialize(array("userid" => $check_user['id'], "username" => $data['username'])), time() + 3600 * 24 * 30, "/");
                                }
                                else
                                {
                ...
                elseif ($_POST['type'] == 2)
                {
                        ...
                        if (empty($arMember))
                        {
                                ...
                                if (!empty($arCompany))
                                {
                                        if ($arCompany['password'] != $data['password'])
                                        {
                                        ...
                                        else
                                        {
                                                //修改登陆信息
                                                $arUserLogin = array("logintime" => time(), "loginip" => $_SERVER["REMOTE_ADDR"]);
                                                M("company")->where("id=" . $arCompany['id'])->save($arUserLogin);

                                                //存进session信息
                                                $_SESSION['cuserinfo'] = array("userid" => $arCompany['id'], "username" => $arCompany['username']);
                                                echo json_encode(array("code" => 200, "msg" => "登陆成功"));
                                                die;
                                        }
                                }
        ...
        public function logout()
        {
                ...
                }elseif(!empty($_SESSION['cuserinfo'])){
