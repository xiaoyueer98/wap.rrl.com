...
function MyShareClass() {
    ...
    L = {
        ...
        shareimgsrc: "http://m.renrenlie.com/Public/images/wep_logo.png"
    },
    K = {
        iTitle: "",
        iContent: "",
    ...
    function M() {
        var aa = readConfig.share.targeturl,
        Z = "";
        if ( !! aa == false) {
            return false
        }
    ...
    function d() {
        ...
        if (readConfig.isArt) {
            ...
            if (isqqBrowser || isucBrowser) {
                ...
                Z.push("<ul>");
                // Z.push('<li class="shareText">谁的丰盛的分享</li>');
                Z.push("<li>");
                Z.push("<ul style='border:none;'>");
                Z.push("<li></li>");
                ...
                Z.push('<li><span class="' + t.findClass.shareIcon + ' splat_ico friend_small aa" data-platform="' + G.friend[0] + '"  data-sudaclick="public_wechatfriends"></span></li>');
                Z.push('<li><span class="' + t.findClass.shareIcon + ' splat_ico fcircle_small bb" data-platform="' + G.fsircle[0] + '"  data-sudaclick="public_wechattimeline"></span></li>');
                Z.push("<li></li>");
                Z.push("</ul>");
                Z.push("</li>");
                Z.push("</ul>");
    ...
    function S() {
        if (typeof __pkeys != "undefined" && typeof __pValue != "undefined") {
            ...
            aa = ab + "show" + ae + "getPraise",
            Z = ab + "add" + ae + "addPraise",
    ...
    window.getPraise = function(aa) {
        var Z = aa.count > 0 ? (aa.count > 10000 ? (parseInt(aa.count / 10000) + "万") : aa.count) : "赞";
        $("." + t.findClass.opPraise).html(Z).data("loading", "no");
        D.voteNum = aa.count
    };
    window.addPraise = function(Z) {
        if (Z.status == 0) {
            $("." + t.findClass.opPraise).each(function() {
                if ($("." + t.findClass.opPraise).data("loading") == "no") {
                    var aa = Z.count > 0 ? (Z.count > 10000 ? (parseInt(Z.count / 10000) + "万") : Z.count) : "赞";
                    $("." + t.findClass.opPraise).text(aa).addClass("on");
                    D.voteNum = Z.count
                }
            })
        } else {}
    ...
    function A(af, Z) {
        ...
        if (__userConfig__.__isLogin) {
            ...
            var ab = "http://o.my.sina.cn/favorite?uid=" + ac + "&docid=" + aa + "&backurl=" + ae + "&ch=" + cur_domain + "&csrftime=" + __colleid + "&csrfcode=" + __collekey + "&channel=" + __domain + ag;
            p(ab)
        } else {
            if (Z && q.isAdd) {
                l(W.login, A, af, true)
    ...
    window.addFavorCallback = function(aa) {
        var ab = $("." + t.findClass.addFavor).eq(0);
        ab.data("loading", "no");
        if (aa && aa.code == 1) {
            ab.addClass("on");
            ab.addClass("on").html("已收藏");
            var Z = "您已收藏，请到个人中心查看";
            X(Z, false)
        } else {
            if (aa && aa.code == 2) {
    ...
    function x(af) {
        ...
        if (!af) {
            ...
            $("#" + t.findId.shareContentid).on("click tap",
            function() {
                var ai = ab.html();
                X();
                ab.focus()
