...
//                echo "<pre>";var_dump($result);echo "</pre>";die;
        ...
        function resume_status($resumeid, $jobid) {
                ...
                $arAllResume = M("resume")->query("select s.id,s.mobile bt_mobile  from stj_resume r join stj_resume s  on r.id={$resumeid} and s.mobile=r.mobile ");
                ...
                //判断该简历是否已经推荐给该职位
//                $arRecord = M("record")->where($where . " AND j_id={$jobid} and (audstart=1 or audstart=3 or audstart=5 or audstart=6) and status !=1")->find();
                $arRecord = M("record")->where($where . " AND j_id={$jobid} ")->find();
                if ($arRecord) {
                        return "disabled";
                }
                //判断该简历是否已经推荐给该企业的一个职位
...
//                $isThreeCompany = M("record")->query("select *  from stj_record  r join stj_job b on  r.j_id = b.id  and " . $where . "  and (audstart=1 or audstart=3 or audstart=5 or audstart=6) and status !=1 group by  b.cpid;");
         ...
         * 判断该简历是否已经存在
         */
        public function judge_resume_mobile(){
                
                $mobile = $_POST['mobile'];
                //查看与该候选人电话号码相同的候选人的简历
                $arAllResume = M("resume")->query("select *  from stj_resume where mobile='{$mobile}' and isshow=1 ");
                if(empty($arAllResume)){
                        echo json_encode(array("code"=>200,"msg"=>"ok"));
                        die;
                }else{
                        echo json_encode(array("code"=>500,"msg"=>"no"));
                        die;
                }
        }
