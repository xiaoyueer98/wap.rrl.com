...
class CustomizedAction extends Action {
        ...
        function index() {
                ...
                $lArr = array(
                    ...
                    array("name" => "私人订制", "url" => "/index.php?s=/Customized/index", "img" => "/Public/new-images/head-icon/srdz.png"),
                );
                if(isset($_GET['from'])){
                       $from = trim($_GET['from']);
                       $ip = $_SERVER['REMOTE_ADDR'];
                       $isExit = M("click_count")->where("tag='$from' and ip='$ip'")->find();
                       if(empty($isExit)){
                               $arClickCount = array();
                               $arClickCount['tag'] = $from;
                               $arClickCount['ip'] = $ip;
                               $arClickCount['created_at'] = time();
                                M("click_count")->add($arClickCount);
                       }
                }
                //查看分享信息
                $arShare = D("ShareInfo")->get_info("share1");
                if(empty($arShare)){
                      $arShare['wapurl']  = "http://m.renrenlie.com/index.php?s=/Customized/index.html";
                ...
                $this->assign("domain","http://m.renrenlie.com");
                $this->assign("lArr", $lArr);
                $this->assign("header_title", "订制工作机会");
